...
func submitRecords(client pb.DnsStorageClient) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for record := range records {
			...
			_, err := client.SaveDomainAddress(r.Context(), &pb.NewDomainAddress{Domain: record.Domain, Address: addr})
			if err != nil {
				log.Print(err)
...
func getRecord(client pb.DnsStorageClient) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		addr, err := client.GetDomainAddress(ctx, &pb.Domain{Domain: q.Get("domain")})
...
func CreateRouter(c pb.DnsStorageClient) *mux.Router {
	r := mux.NewRouter()
